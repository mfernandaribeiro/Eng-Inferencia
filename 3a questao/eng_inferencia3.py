from base_conhecimento import knowledge_base_q3
class Risk:
      ...
      while True:
          if isinstance(current_node, dict):
              for question, next_nodes in list(current_node.items()):
                  ...
                  response = self.ask_question(f"Você possui {question}?")
                  if response != "por que":
                ...
                print(f"O risco para o empréstimo é {current_node}.")
                return
    ...
    def run(self):
        while True:
            print("Seja bem vindo, por favor informe os dados necessários para a consulta!")
            self.infer(self.knowledge_base)

            while True:
                replay_choice = input("\nEscolha uma opção:\n(j) Realiza a consulta novamente\n(r) Ver como chegou ao resultado\n(s) Encerrar a consulta\nEscolha uma opção: ").lower()
                if replay_choice == 'j':
                    break
                    ...
                    print("Opção inválida. Por favor, escolha 'j' para  consultar novamente , 'r' para ver o histórico ou 's' para sair.")
